...
- **Chi-Square Test of Independence**: Evaluated the relationship between Geographic Region and 2-Day Delivery SLA Achievement. Result: $\chi^2 = 12610.26$, $p < 0.001$. This confirms a highly statistically significant association, demonstrating that SLA failure is geographically clustered rather than random.
- **One-Way ANOVA**: Tested equality of mean shipping costs across the 6 geographic regions. Result: $F$-statistic $= 1574.61$, $p < 0.001$. Confirms significant cost variance across regions under single-warehouse fulfillment.
- **Pearson Correlation**: Evaluated distance (km) vs. shipping cost ($USD$). Result: $r = 0.5586$ ($p < 0.001$), demonstrating a strong positive linear relationship between transit distance and freight expenditure.
...
- **Demand-Weighted Center of Gravity**: Latitude = `38.1599°`, Longitude = `-93.151°` (Corresponding to the Saint Louis / Springfield, MO metropolitan corridor).
...
| :--- | :--- | :--- | :--- | :--- |
| **Linear Regression** | $4.303$ | $6.416$ | `0.9175` | `91.7%` |
| **Random Forest** | $1.29$ | $1.629$ | `0.9947` | `99.5%` |
| **Gradient Boosting** | $1.29$ | $1.627$ | `0.9947` | `99.5%` |
